...
def main():
    ...
    # ============================================================
    print("\n" + "=" * 70)
    print("步骤1: 基准方法验证 (VNA/eVNA/aVNA)")
    print("=" * 70)
    ...
    # Check for innovation methods to validate
    innovation_methods = [
        f for f in os.listdir(output_dir)
        if f.endswith('_summary.csv') and not f.startswith('benchmark')
    ]

    if innovation_methods:
        print(f"\nFound {len(innovation_methods)} innovation method results")

        innovation_summary = []
        for csv_file in innovation_methods:
