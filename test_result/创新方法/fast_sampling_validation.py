...
def run_sampled_month(model_class, year_month, sample_days=10, **model_params):
    """运行采样月份验证(采样sample_days天)"""
    ...
    if month in [1, 3, 5, 7, 8, 10, 12]:
        ...
        days_in_month = 29 if year == 2020 else 28

    # 均匀采样sample_days天
    sample_indices = np.linspace(1, days_in_month, sample_days, dtype=int)

    all_y_true = []
    all_y_pred = []

    for day in sample_indices:
        ...
        try:
            ...
            print(f"    {date_str}: R2={metrics['R2']:.4f}")
...
def main():
    print("="*70)
    print("PM2.5 CMAQ Fusion Methods - Fast Sampling Validation")
    print("="*70)
    print(f"\nBaseline: R2 >= {BENCHMARK_R2}")
    ...
    # Stage 3: July (sampled 10 days)
    print("\n" + "="*70)
    print("Stage 3: July 2020 (sampled 10 days)")
    print("="*70)
    ...
    for method_name, model_class, params in [
        ...
        print(f"\n--- {method_name} ---")
        metrics = run_sampled_month(model_class, '2020-07', sample_days=10, **params)
        print(f"  July (sampled): R2={metrics['R2']:.4f}, RMSE={metrics['RMSE']:.2f}, MB={metrics['MB']:.2f}")

        stage_pass = metrics['R2'] >= BENCHMARK_R2
        print(f"  Pass: {stage_pass}")

        results[f'{method_name}_stage3'] = {
            'stage': 'July 2020 (sampled)',
            'R2': float(metrics['R2']),
    ...
    # Stage 4: December (sampled 10 days)
    print("\n" + "="*70)
    print("Stage 4: December 2020 (sampled 10 days)")
    print("="*70)
    ...
    for method_name, model_class, params in [
        ...
        print(f"\n--- {method_name} ---")
        metrics = run_sampled_month(model_class, '2020-12', sample_days=10, **params)
        print(f"  December (sampled): R2={metrics['R2']:.4f}, RMSE={metrics['RMSE']:.2f}, MB={metrics['MB']:.2f}")

        stage_pass = metrics['R2'] >= BENCHMARK_R2
        print(f"  Pass: {stage_pass}")

        results[f'{method_name}_stage4'] = {
            'stage': 'December 2020 (sampled)',
            'R2': float(metrics['R2']),
            'MAE': float(metrics['MAE']),
            'RMSE': float(metrics['RMSE']),
            'MB': float(metrics['MB']),
    ...
    # Load previous results for stages 1 and 2
    import json
    for method_name in ['PG-STGAT', 'VCFFM']:
        for stage_id in [1, 2]:
            path = f'{output_dir}/{method_name}_stage{stage_id}_results.json'
            if os.path.exists(path):
                with open(path) as f:
                    data = json.load(f)
                    results[f'{method_name}_stage{stage_id}'] = data
    ...
    # Re-check stage 1 and 2 from loaded results
