"""
GradientBoostingEnsemble - Gradient Boosting Residual + GPR Hybrid Ensemble
===========================================================================
创新点:
1. 使用GradientBoosting建模残差，替代MLP
2. 结合GPR的统计建模能力
3. 与eVNA进行三层集成
"""
...
def run_gradient_boosting_ensemble_ten_fold(selected_day='2020-01-01'):
    """
    运行GradientBoostingEnsemble十折交叉验证
    """
    print("="*60)
    print("GradientBoostingEnsemble Ten-Fold Cross Validation")
    print("="*60)
    ...
    for fold_id in range(1, 11):
        ...
        # === 2. GradientBoosting残差建模 ===
        gb = GradientBoostingRegressor(
            n_estimators=100, max_depth=3, learning_rate=0.1,
            subsample=0.8, random_state=42
        )
        gb.fit(X_train, residual_poly)
        gb_pred = gb.predict(X_test)
        gb_rk_pred = pred_poly + gb_pred
    ...
    gb_rk_all = np.concatenate([results[f]['gb_rk'] for f in range(1, 11) if results[f]])
    ...
    print(f"  GB-RK: {compute_metrics(true_all, gb_rk_all)['R2']:.4f}")
    print(f"  eVNA: {compute_metrics(true_all, evna_all)['R2']:.4f}")

    # 优化Ensemble权重
    print("\n=== Optimizing Triple Ensemble (RK-Poly + GB-RK + eVNA) ===")
    best_r2 = -np.inf
    best_weights = None
    weight_results = []
    ...
    for w1 in np.arange(0, 1.05, 0.05):
        for w2 in np.arange(0, 1.05 - w1, 0.05):
            ...
            ensemble_pred = w1 * rk_poly_all + w2 * gb_rk_all + w3 * evna_all
            metrics = compute_metrics(true_all, ensemble_pred)

            weight_results.append({
                'w1_rk_poly': w1, 'w2_gb': w2, 'w3_evna': w3,
    ...
    print(f"\nBest weights: RK-Poly={best_weights[0]:.2f}, GB-RK={best_weights[1]:.2f}, eVNA={best_weights[2]:.2f}")
    print(f"Best R2: {best_r2:.4f}")

    # 最终评估
    final_pred = best_weights[0] * rk_poly_all + best_weights[1] * gb_rk_all + best_weights[2] * evna_all
    final_metrics = compute_metrics(true_all, final_pred)

    print("\n" + "="*60)
    print("Final GradientBoostingEnsemble Results")
    print("="*60)
    print(f"Optimal weights: RK-Poly={best_weights[0]:.2f}, GB-RK={best_weights[1]:.2f}, eVNA={best_weights[2]:.2f}")
    ...
    result_df = pd.DataFrame([{
        'method': 'GradientBoostingEnsemble',
        'w1_rk_poly': best_weights[0],
        'w2_gb': best_weights[1],
        'w3_evna': best_weights[2],
        **final_metrics
    }])
    result_df.to_csv(f'{output_dir}/GradientBoostingEnsemble_summary.csv', index=False)

    weight_df = pd.DataFrame(weight_results)
    weight_df.to_csv(f'{output_dir}/GradientBoostingEnsemble_weight_search.csv', index=False)
...
if __name__ == '__main__':
    metrics, weights = run_gradient_boosting_ensemble_ten_fold('2020-01-01')
