...
====================================================
创新点:
1. 使用Stacking方法组合多个基础模型
2. 使用Ridge回归作为元学习器
3. 基础模型：RK-Poly, eVNA, RK-OLS
"""
...
def run_stacking_ensemble_ten_fold(selected_day='2020-01-01'):
    """
    运行StackingEnsemble十折交叉验证
    """
    print("="*60)
    print("StackingEnsemble Ten-Fold Cross Validation")
    print("="*60)
    ...
    # 训练元学习器
    X_meta = np.column_stack([oof_rk_poly, oof_rk_ols, oof_evna])
    meta_model = Ridge(alpha=1.0)
    meta_model.fit(X_meta, oof_true)
    meta_pred = meta_model.predict(X_meta)

    print(f"  Meta-learner coefficients: RK-Poly={meta_model.coef_[0]:.3f}, RK-OLS={meta_model.coef_[1]:.3f}, eVNA={meta_model.coef_[2]:.3f}")
    print(f"  Meta-learner intercept: {meta_model.intercept_:.3f}")
    print(f"  Stacking R2: {compute_metrics(oof_true, meta_pred)['R2']:.4f}")
    ...
    for w1 in np.arange(0, 1.01, 0.05):
        for w2 in np.arange(0, 1.01 - w1, 0.05):
            ...
            ensemble_pred = w1 * rk_poly_all + w2 * rk_ols_all + w3 * evna_all
    ...
    print(f"\nBest simple ensemble: RK-Poly={best_weights[0]:.2f}, RK-OLS={best_weights[1]:.2f}, eVNA={best_weights[2]:.2f}")
    print(f"Best simple R2: {best_r2:.4f}")
    ...
    if stacking_metrics['R2'] > best_r2:
        ...
        final_metrics = compute_metrics(true_all, best_weights[0] * rk_poly_all + best_weights[1] * rk_ols_all + best_weights[2] * evna_all)
        method_name = 'SimpleWeighted'
        final_pred = best_weights[0] * rk_poly_all + best_weights[1] * rk_ols_all + best_weights[2] * evna_all

    print("\n" + "="*60)
    print("Final StackingEnsemble Results")
    print("="*60)
    ...
    result_df.to_csv(f'{output_dir}/StackingEnsemble_summary.csv', index=False)
...
if __name__ == '__main__':
    metrics, method = run_stacking_ensemble_ten_fold('2020-01-01')
