"""
NNResidualEnsemble - Neural Network Residual + GPR Hybrid Ensemble
====================================================================
创新点:
1. 使用MLP神经网络建模残差，学习复杂的非线性空间模式
2. 结合GPR的统计建模能力
3. 与eVNA进行三层集成
"""
...
def run_nn_residual_ensemble_ten_fold(selected_day='2020-01-01'):
    """
    运行NNResidualEnsemble十折交叉验证
    """
    print("="*60)
    print("NNResidualEnsemble Ten-Fold Cross Validation")
    print("="*60)
    ...
    for fold_id in range(1, 11):
        ...
        # === 2. MLP残差建模 ===
        # 归一化坐标
        lon_mean, lon_std = X_train[:, 0].mean(), X_train[:, 0].std()
        lat_mean, lat_std = X_train[:, 1].mean(), X_train[:, 1].std()
        X_train_norm = (X_train - [lon_mean, lat_mean]) / [lon_std, lat_std]
        X_test_norm = (X_test - [lon_mean, lat_mean]) / [lon_std, lat_std]

        mlp = MLPRegressor(hidden_layer_sizes=(32, 16), activation='relu', solver='adam',
                           max_iter=500, early_stopping=True, random_state=42)
        mlp.fit(X_train_norm, residual_poly)
        mlp_pred = mlp.predict(X_test_norm)
        nn_residual_pred = pred_poly + mlp_pred
    ...
    nn_residual_all = np.concatenate([results[f]['nn_residual'] for f in range(1, 11) if results[f]])
    ...
    print(f"  NN-Residual: {compute_metrics(true_all, nn_residual_all)['R2']:.4f}")
    print(f"  eVNA: {compute_metrics(true_all, evna_all)['R2']:.4f}")

    # 优化3方法Ensemble权重
    print("\n=== Optimizing Triple Ensemble (RK-Poly + NN-Residual + eVNA) ===")
    best_r2 = -np.inf
    best_weights = None
    weight_results = []
    ...
    for w1 in np.arange(0, 1.05, 0.05):
        for w2 in np.arange(0, 1.05 - w1, 0.05):
            ...
            ensemble_pred = w1 * rk_poly_all + w2 * nn_residual_all + w3 * evna_all
            metrics = compute_metrics(true_all, ensemble_pred)

            weight_results.append({
                'w1_rk_poly': w1, 'w2_nn': w2, 'w3_evna': w3,
    ...
    print(f"\nBest weights: RK-Poly={best_weights[0]:.2f}, NN-Residual={best_weights[1]:.2f}, eVNA={best_weights[2]:.2f}")
    print(f"Best R2: {best_r2:.4f}")

    # 最终评估
    final_pred = best_weights[0] * rk_poly_all + best_weights[1] * nn_residual_all + best_weights[2] * evna_all
    final_metrics = compute_metrics(true_all, final_pred)

    print("\n" + "="*60)
    print("Final NNResidualEnsemble Results")
    print("="*60)
    print(f"Optimal weights: RK-Poly={best_weights[0]:.2f}, NN-Residual={best_weights[1]:.2f}, eVNA={best_weights[2]:.2f}")
    ...
    result_df = pd.DataFrame([{
        'method': 'NNResidualEnsemble',
        'w1_rk_poly': best_weights[0],
        'w2_nn': best_weights[1],
        'w3_evna': best_weights[2],
        **final_metrics
    }])
    result_df.to_csv(f'{output_dir}/NNResidualEnsemble_summary.csv', index=False)

    weight_df = pd.DataFrame(weight_results)
    weight_df.to_csv(f'{output_dir}/NNResidualEnsemble_weight_search.csv', index=False)
...
if __name__ == '__main__':
    metrics, weights = run_nn_residual_ensemble_ten_fold('2020-01-01')
