"""
PolyEnsemble - Polynomial RK + eVNA Ensemble
============================================
组合多项式残差克里金和eVNA
"""
...
def run_poly_ensemble_ten_fold(selected_day='2020-01-01'):
    """
    运行PolyEnsemble十折交叉验证
    """
    print("="*60)
    print("PolyEnsemble Ten-Fold Cross Validation")
    print("="*60)
    ...
    # 优化Ensemble权重
    print("\n=== Optimizing Ensemble (RK-Poly + eVNA) ===")
    best_r2 = -np.inf
    best_weights = None
    weight_results = []
    ...
    for w1 in np.arange(0, 1.05, 0.05):
        w2 = round(1.0 - w1, 2)
        ensemble_pred = w1 * rk_poly_all + w2 * evna_all
        metrics = compute_metrics(true_all, ensemble_pred)

        weight_results.append({
            'w1_rk_poly': w1, 'w2_evna': w2,
    ...
    print(f"\nBest weights: RK-Poly={best_weights[0]:.2f}, eVNA={best_weights[1]:.2f}")
    print(f"Best R2: {best_r2:.4f}")

    # 最终评估
    final_pred = best_weights[0] * rk_poly_all + best_weights[1] * evna_all
    final_metrics = compute_metrics(true_all, final_pred)

    print("\n" + "="*60)
    print("Final PolyEnsemble Results")
    print("="*60)
    print(f"Optimal weights: RK-Poly={best_weights[0]:.2f}, eVNA={best_weights[1]:.2f}")
    ...
    result_df.to_csv(f'{output_dir}/PolyEnsemble_summary.csv', index=False)

    weight_df = pd.DataFrame(weight_results)
    weight_df.to_csv(f'{output_dir}/PolyEnsemble_weight_search.csv', index=False)
...
if __name__ == '__main__':
    metrics, weights = run_poly_ensemble_ten_fold('2020-01-01')
