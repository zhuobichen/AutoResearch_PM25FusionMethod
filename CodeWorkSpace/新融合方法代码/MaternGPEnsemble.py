"""
MaternGPEnsemble - Matérn Kernel GPR + eVNA Ensemble
====================================================
创新点:
1. 使用Matérn 3/2核替代RBF核，更灵活的空间相关性建模
2. 多项式阶数从2提升到3，更精确的非线性捕捉
3. 细粒度权重搜索(0.01步长)
"""
...
def run_matern_gp_ensemble_ten_fold(selected_day='2020-01-01'):
    """
    运行MaternGPEnsemble十折交叉验证
    """
    print("="*60)
    print("MaternGPEnsemble Ten-Fold Cross Validation")
    print("="*60)
    ...
    # 定义Matérn核函数 (nu=1.5 即 Matern 3/2)
    ...
    for fold_id in range(1, 11):
        ...
        # === 1. RK-Poly-3 (三次多项式) with Matérn GPR ===
        ...
        # 使用Matérn核GPR
        gpr_poly = GaussianProcessRegressor(kernel=kernel, n_restarts_optimizer=3, alpha=0.1, normalize_y=True)
    ...
    print(f"  RK-Poly3-Matern: {compute_metrics(true_all, rk_poly3_all)['R2']:.4f}")
    print(f"  eVNA: {compute_metrics(true_all, evna_all)['R2']:.4f}")

    # 细粒度权重优化 (0.01步长)
    print("\n=== Fine-grained Ensemble Optimization (step=0.01) ===")
    best_r2 = -np.inf
    best_weights = None
    weight_results = []
    ...
    for w1 in np.arange(0, 1.01, 0.01):
        w2 = round(1.0 - w1, 2)
        ensemble_pred = w1 * rk_poly3_all + w2 * evna_all
        metrics = compute_metrics(true_all, ensemble_pred)

        weight_results.append({
            'w1_rk_poly3': w1, 'w2_evna': w2,
    ...
    print(f"\nBest weights: RK-Poly3-Matern={best_weights[0]:.2f}, eVNA={best_weights[1]:.2f}")
    print(f"Best R2: {best_r2:.4f}")

    # 最终评估
    final_pred = best_weights[0] * rk_poly3_all + best_weights[1] * evna_all
    final_metrics = compute_metrics(true_all, final_pred)

    print("\n" + "="*60)
    print("Final MaternGPEnsemble Results")
    print("="*60)
    print(f"Optimal weights: RK-Poly3-Matern={best_weights[0]:.2f}, eVNA={best_weights[1]:.2f}")
    ...
    result_df = pd.DataFrame([{
        'method': 'MaternGPEnsemble',
        'w1_rk_poly3': best_weights[0],
        'w2_evna': best_weights[1],
        **final_metrics
    }])
    result_df.to_csv(f'{output_dir}/MaternGPEnsemble_summary.csv', index=False)

    weight_df = pd.DataFrame(weight_results)
    weight_df.to_csv(f'{output_dir}/MaternGPEnsemble_weight_search.csv', index=False)
...
if __name__ == '__main__':
    metrics, weights = run_matern_gp_ensemble_ten_fold('2020-01-01')
