...
def ten_fold_gvna_single_day(selected_day):
    ...
    for fold_id in range(1, 11):
        ...
        model = gVNA(k=30, p=2, lambda_bg=15.0)
        model.fit(
            train_df['Lon'].values,
            train_df['Lat'].values,
...
def run_stage(stage_name, start_date, end_date):
    sep = "=" * 60
    print(sep)
    print("gVNA Stage: " + stage_name + " (" + start_date + " ~ " + end_date + ")")
    print(sep)
    ...
    for i, date_str in enumerate(date_list):
        ...
        result = ten_fold_gvna_single_day(date_str)
    ...
    print("gVNA Result: R2=" + str(metrics['R2']) + ", RMSE=" + str(metrics['RMSE']) + ", MB=" + str(metrics['MB']))
    ...
    print("Innovation: R2>=" + str(base['R2']+0.01) + "? " + r2_str + "  RMSE<=" + str(base['RMSE']) + "? " + rmse_str + "  |MB|<=" + str(base['MB']) + "? " + mb_str)
...
def main():
    print("=" * 70)
    print("gVNA Multi-Stage Validation (bias-only + similarity weight)")
    print("=" * 70)
    ...
    output_file = OUTPUT_DIR + '/gVNA_all_stages.json'
